...
Page.onReady = function () {
  ...
  if ('BroadcastChannel' in window) {
    emplistChannel = new BroadcastChannel('emplist_channel');
    emplistChannel.onmessage = function (event) {
      const msg = event.data || {};
      if (msg.type === 'EMPLIST_CONFIG_UPDATED') {
        Page.applyColumnConfig();
      }
    };
  }
...
Page.anchorSelectColumnsClick = function ($event, widget) {
  App.redirectTo('mgremplistconfig');
};
