...
/**
 * onSuccess handler for svScheduleList (REST GET).
 *
 * Groups the returned employee schedule list by position. Each employee may
 * span multiple weeklyShifts days, each with multiple shift objects. The
 * function collects all unique positions across all employees/days/shifts,
 * then for each position produces:
 *   { position: string, employees: [ ...cloned employee objects that have
 *     at least one shift with that position in the current week ] }
 *
 * The result is stored in Page.Variables.transformedScheduleVar via setData().
 * The original svScheduleList.dataSet is never mutated.
 */
Page.svScheduleListonSuccess = function (variable, data, options) {
    // Safely read the raw list — prefer data.dataValue, fall back to variable.dataSet
    var rawList = (data && data.dataValue) ? data.dataValue : (variable.dataSet || []);
    ...
    // Deep clone to avoid mutating the original dataset
    var clonedList = JSON.parse(JSON.stringify(rawList));

    // Collect all unique position names across all employees → weeklyShifts → shifts
    var positionSet = {};
    _.forEach(clonedList, function (employee) {
        _.forEach(employee.weeklyShifts, function (dayEntry) {
            _.forEach(dayEntry.shifts, function (shift) {
                if (shift.position) {
                    positionSet[shift.position] = true;
                }
            });
        });
    });

    var uniquePositions = Object.keys(positionSet);

    // For each unique position, find employees that have at least one shift
    // with that position anywhere in their weeklyShifts
    var grouped = _.map(uniquePositions, function (positionName) {
        var matchingEmployees = _.filter(clonedList, function (employee) {
            return _.some(employee.weeklyShifts, function (dayEntry) {
                return _.some(dayEntry.shifts, function (shift) {
                    return shift.position === positionName;
                });
            });
        });
    ...
    Page.Variables.transformedScheduleVar.setData(grouped);
    console.log("uped by positiongro", Page.Variables.transformedScheduleVar.dataSet);
};
